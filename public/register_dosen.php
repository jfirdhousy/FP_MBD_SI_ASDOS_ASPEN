...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validasi sederhana
    if (empty($nip) || empty($nama_dosen) || empty($email) || empty($password) || empty($departemen_id)) {
        ...
        try {
            ...
            $stmt_check = $conn->prepare("SELECT COUNT(*) FROM dosen WHERE nip = :nip OR email = :email");
            $stmt_check->bindParam(':nip', $nip);
            $stmt_check->bindParam(':email', $email);
            $stmt_check->execute();
            if ($stmt_check->fetchColumn() > 0) {
                $error = "NIP atau Email sudah terdaftar.";
            } else {
                $stmt = $conn->prepare("INSERT INTO dosen (nip, nama_dosen, email, password, no_telp, departemen_id) VALUES (:nip, :nama_dosen, :email, :password, :no_telp, :departemen_id)");
                ...
                if ($stmt->execute()) {
                    ...
                    // header("Location: " . BASE_URL . "login.php?registered=dosen"); // Bisa diarahkan setelah registrasi
                    // exit();
                } else {
        ...
        <div class="col-md-7">
            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white text-center">
                    <h4 class="mb-0">Registrasi Akun Dosen</h4>
                </div>
                <div class="card-body">
