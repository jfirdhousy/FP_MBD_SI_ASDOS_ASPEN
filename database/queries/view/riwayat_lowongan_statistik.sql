...
SELECT
    ...
    SUM(CASE WHEN lr.status_lamaran = 'Diterima' THEN 1 ELSE 0 END) AS lamaran_diterima,
    SUM(CASE WHEN lr.status_lamaran = 'Ditolak' THEN 1 ELSE 0 END) AS lamaran_ditolak,
    SUM(CASE WHEN lr.status_lamaran = 'Pending' THEN 1 ELSE 0 END) AS lamaran_pending_saat_tutup -- Jumlah pending saat lowongan berakhir
FROM
    lowongan l
JOIN
